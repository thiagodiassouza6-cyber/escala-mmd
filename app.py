...
I18N = {
    "PT": {
        "lang_code": "pt-BR", "titulo": "🚀 MMD | Portal de Gestão", "login_tit": "Portal de Gestão MMD",
        "usuario": "Usuário", "senha": "Senha", "acessar": "Acessar Painel", "estrutura_tit": "👥 Estrutura de Times",
        "roteiro_ter": "📝 Roteiro Terça: Práticas + Iniciativas", "roteiro_qui": "📝 Roteiro Quinta: Lead Time + SLA",
        "tab_escalas": "📅 Escalas", "tab_ferias": "🌴 Férias", "ferias_tit": "🌴 Planejamento de Férias Integrado",
        "reg_periodo": "Registrar Período", "colaborador": "Colaborador:", "usuario_log": "Seu Usuário (Obrigatório):",
        "dt_inicio": "Data de Início:", "dt_fim": "Data de Término:", "obs": "Observação:", "btn_salvar": "💾 Salvar no Sheets",
        "grade_tit": "Grade de Disponibilidade", "sel_mes": "Mês:", "sel_ano": "Ano:", "filtro_equipe": "Filtrar por Equipe:",
        "viz_ocupacao": "Ocupação: {equipe} em {mes}/{ano}", "log_tit": "📋 Próximas Férias (Cronológico)",
        "err_user": "Por favor, informe o seu usuário.", "err_data": "A data de início não pode ser maior que a data de término.",
        "err_conflito": "❌ Erro de Agendamento: {nome} já tem férias marcadas na equipe '{equipe}' nesse período. Não é permitido duas pessoas da mesma equipe fora ao mesmo tempo.",
        "sucesso": "✅ Férias registradas!", "livre": "Livre", "dias_semana_curto": ["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"],
        "meses": ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"],
        "buscar": "🔍 Buscar por Apresentador:", "todos": "Todos", "semana": "Semana:", "backup": "🔄 Backup", "flash_m": "Flash Manhã"
    },
    "ES": {
        "lang_code": "es-ES", "titulo": "🚀 MMD | Portal de Gestión", "login_tit": "Portal de Gestión MMD",
        "usuario": "Usuario", "senha": "Contraseña", "acessar": "Acceder al Panel", "estrutura_tit": "👥 Estructura de Equipos",
        "roteiro_ter": "📝 Guion Martes: Prácticas + Iniciativas", "roteiro_qui": "📝 Guion Jueves: Lead Time + SLA",
        "tab_escalas": "📅 Escalas", "tab_ferias": "🌴 Vacaciones", "ferias_tit": "🌴 Planificación de Vacaciones Integrada",
        "reg_periodo": "Registrar Período", "colaborador": "Colaborador:", "usuario_log": "Tu Usuario (Obligatorio):",
        "dt_inicio": "Fecha de Inicio:", "dt_fim": "Fecha de Finalización:", "obs": "Observación:", "btn_salvar": "💾 Guardar en Sheets",
        "grade_tit": "Cuadrícula de Disponibilidad", "sel_mes": "Mes:", "sel_ano": "Año:", "filtro_equipe": "Filtrar por Equipo:",
        "viz_ocupacao": "Ocupación: {equipe} en {mes}/{ano}", "log_tit": "📋 Próximas Vacaciones (Cronológico)",
        "err_user": "Por favor, informe su usuario.", "err_data": "La fecha de inicio no puede ser mayor que la fecha de finalización.",
        "err_conflito": "❌ Error de Agendamiento: {nome} ya tiene vacaciones en el equipo '{equipe}' en este período. No se permite dos personas del mismo equipo fuera al mismo tiempo.",
        "sucesso": "✅ ¡Vacaciones registradas!", "livre": "Libre", "dias_semana_curto": ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"],
        "meses": ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"],
        "buscar": "🔍 Buscar por Presentador:", "todos": "Todos", "semana": "Semana:", "backup": "🔄 Backup", "flash_m": "Flash Mañana"
    }
}
...
@st.cache_data
def gerar_escala_estatica(nomes):
    # Usamos uma lista fixa para garantir que a ordem não mude nunca
    lista_fixa = sorted(nomes)
    dias_range = pd.date_range(datetime(2026, 1, 1), datetime(2026, 12, 31), freq='B')
    escala = []
    for i, dia in enumerate(dias_range):
        ap_idx = i % len(lista_fixa)
        ap_m = lista_fixa[ap_idx]
        escala.append({
            "Semana": dia.isocalendar()[1], "Data": dia.strftime("%d/%m/%Y"),
            "Reunião": t["flash_m"], "Apresentador": ap_m,
            "Backup": MAPA_REFERENCIA.get(ap_m, "Sem Backup")
        })
...
if check_login():
    ...
    with tab_escala:
        st.title(t["titulo"])
        nomes_ativos = sorted(list(MAPA_REFERENCIA.keys()))
        df_total = gerar_escala_estatica(nomes_ativos)
        busca = st.selectbox(t["buscar"], [t["todos"]] + nomes_ativos)
        ...
        for dt, gp in df_s.groupby("Data", sort=False):
            ...
            for i, (_, r) in enumerate(gp.iterrows()):
                with cols[i]:
                    st.markdown(f'<div style="background-color:#f0f2f6;padding:15px;border-radius:10px;border-left:5px solid #ff4b4b;"><b>{r["Reunião"]}</b><br><span style="font-size:18px;font-weight:bold;">🏆 {r["Apresentador"]}</span><br><small>{t["backup"]}: {r["Backup"]}</small></div>', unsafe_allow_html=True)
    ...
    with tab_ferias:
        ...
        if sh:
                ...
                with st.form("form_ferias"):
                    ...
                    if st.form_submit_button(t["btn_salvar"]):
                        if not user_login: st.error(t["err_user"])
                            ...
                            conflito = df_v[(df_v['Equipe'] == torre_sel) & (d_ini <= df_v['Data Final']) & (d_fim >= df_v['Data Início'])]
            ...
            if not df_ferias.empty:
                ...
                st.dataframe(df_log.sort_values(by='Data Início DT', ascending=True).drop(columns=['Data Início DT']), use_container_width=True, hide_index=True)
